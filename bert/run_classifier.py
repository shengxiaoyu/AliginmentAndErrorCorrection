...
class SimilarPhraseProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
    ...
    def _create_examples(self,lines,set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            text_a = tokenization.convert_to_unicode(line[0])
            text_b = tokenization.convert_to_unicode(line[1])
            examples.append(InputExample(guid=guid, text_a=text_a, text_b=text_b, label=line[2]))
        return examples
...
def main(_):
  ...
  processors = {
      ...
      "mrpc": MrpcProcessor,
      "xnli": XnliProcessor,
      "similar": SimilarPhraseProcessor,
  }
